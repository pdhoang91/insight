...
const Navbar = () => {
  ...
  const handleLogout = () => {
    ...
    setUser(null);
    setDropdownOpen(false);
    setMobileMenuOpen(false); // Đóng menu di động
    router.push('/'); // Chuyển hướng về trang chủ sau khi đăng xuất
  };
  ...
  const handleSearchSubmit = (e) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      setMobileMenuOpen(false); // Đóng menu di động sau khi tìm kiếm
      router.push(`/search?q=${encodeURIComponent(searchQuery.trim())}`);
      setSearchQuery('');
    }
  };
        ...
        <div className="fixed top-16 left-0 w-full bg-white border-b border-gray-300 z-20 md:hidden">
          <div className="flex flex-col p-4 space-y-4">
            ...
            {(isCreatePostPage || isEditPostPage) ? (
              <span
                onClick={handleSubmitPostClick}
                className="px-2 py-2 bg-blue-500 text-white rounded-full hover:bg-blue-600 transition-colors cursor-pointer text-center"
              >
                Publish
              </span>
            ) : (
              <span
                type="button"
                onClick={handleCreatePostClick}
                className="px-2 py-2 bg-green-500 text-white rounded-full hover:bg-green-600 transition-colors cursor-pointer text-center"
              >
                Write
              </span>
            )}
            ...
            {user ? (
              ...
              <button
                onClick={() => {
                  setModalOpen(true);
                  setMobileMenuOpen(false); // Đóng menu khi nhấp vào Sign In
                }}
                className="w-full px-2 py-2 bg-gray-200 text-gray-700 rounded-full hover:bg-gray-300 transition-colors"
              >
                Sign In
              </button>
            )}
