...
const AddCommentForm = ({ onAddComment, postId, user, onCommentAdded, parentId = null, placeholder }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="relative">
      <textarea
        className="w-full bg-medium-bg-secondary rounded-lg text-medium-text-primary placeholder-medium-text-muted p-3 pr-12 resize-none focus:outline-none focus:ring-2 focus:ring-medium-accent-green/50 transition-all duration-200 shadow-sm focus:shadow-md"
      ...
      <button
        type="submit"
        className={`absolute right-3 bottom-3 p-2 rounded transition-all duration-200 ${
          isSubmitting 
