...
func (s *InsightService) ListCategories(req *model.PaginationRequest) ([]*model.CategoryResponse, int64, error) {
	if req.Limit == 0 {
		req.Limit = 10
	}
...
func (s *InsightService) GetTopCategories(req *model.PaginationRequest) ([]*model.CategoryResponse, int64, error) {
	if req.Limit == 0 {
		req.Limit = 10
	}
	...
	if err := s.DBR2.Where("name IN ?", topCategories).
		...
		return nil, 0, appError.InternalServerError("Failed to fetch categories", err)
	}
...
func (s *InsightService) GetPopularCategories(req *model.PaginationRequest) ([]model.CategoryWithCount, int64, error) {
	if req.Limit == 0 {
		req.Limit = 7
	}
	...
	if err := s.DBR2.Raw(query, req.Limit, offset).Scan(&categories).Error; err != nil {
		return nil, 0, appError.InternalServerError("Failed to fetch popular categories", err)
	}
...
func (s *InsightService) CreateCategory(req *model.CreateCategoryRequest) (*model.CategoryResponse, error) {
	...
	if err := category.Create(s.DB); err != nil {
		return nil, appError.InternalServerError("Failed to create category", err)
	}
...
func (s *InsightService) UpdateCategory(id uuid.UUID, req *model.UpdateCategoryRequest) (*model.CategoryResponse, error) {
	...
	if err := category.Update(s.DB); err != nil {
		return nil, appError.InternalServerError("Failed to update category", err)
	}
